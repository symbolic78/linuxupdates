...
check_redhat()
{
   ...
   *)
      ...
      #been a problem
      echo "an error occured with yum"
      exit 1
      ;;
   esac
}
...
{
   $MAIL -s "[REDHAT UPDATES]Server `uname -n` applied updates and restarting core services" $MAILTO << EOF
Server `uname -n` has applied relevant errata and will be restarting its core services in $SLEEP seconds (if any have been scheduled for restart in the SERVICES environment variable in $0). Please check all services are running correctly once the service restart has completed.
...
update_redhat()
{
   ...
   *)
   ...
   case $YUMSTATUS in
   0)
      #looks like yum has completed succesfully, time to restart services
      #generate a random time between 1 minute (60) and 30 minutes (1800)
      SLEEP=$[ ( $RANDOM%1800+60  ) ]
      rm -rf $DOWNLOADDIR/*.rpm
      echo "update completed, server will restart services in $SLEEP seconds"
      mail_service_restart
      sleep $SLEEP
